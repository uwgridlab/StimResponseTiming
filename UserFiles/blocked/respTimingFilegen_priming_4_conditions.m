%% - 5-11-2018 - DJC - response timing file generator for 4 conditions of pilot
...
%  2 = train condition 1
%  3 = train condition 2
%  4 = train condition 3 (priming is on)
...
filename = sprintf('%s_stimTrainDelivery_%s_priming_4_cond.txt',sid,fileNum);
...
filename = sprintf('%s_condition_%s_priming_4_cond.txt',sid,fileNum);
fileID = fopen(filename,'w+');
fprintf(fileID,'%d\r\n',vectorCondRandTotal);
fclose(fileID);
%% deal with priming 
...
primingTotal(vectorCondRandTotal == 4 | vectorCondRandTotal == 5) = 1;

%% write these times for priming

filename = sprintf('%s_primingOption_%s_priming_4_cond.txt',sid,fileNum);
fileID = fopen(filename,'w+');
fprintf(fileID,'%d\r\n',primingTotal);
fclose(fileID);
